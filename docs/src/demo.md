...
| --- | --- |
| `app` | the composition root: a custom `ServiceIdentity` implementing `Info`, and `App` deriving `MetricTree` to mount components in two `flatten`ed groups -- `Standard` (shared names + `service` label) and `Business` (`order_service_` prefix, no label). This module assembles the routing layer from the components' own span metrics |
...
- **Composition shape** -- how the document splices in a metric: `subtree`
  mounts a child under a name segment, `flatten` splices a sub-tree with no
  segment, and `family_by` fans out N dynamic members keyed by a label.
- **Where the numbers come from** -- what backs a metric: a pure-metrics
...
- **Dynamic shape, not a central Family.** `payments` keeps `in_flight` /
  ...
  shape) and a `family_by` view fans them out as
  `order_service_payments_settlements_total{rail="card"}` over a live,
  dynamic map -- a rail added at runtime shows up on the next scrape with no
  extra wiring.
