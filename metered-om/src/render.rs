...
use crate::encoder::{
    FamilyGroup, HistogramProfile, group_families, histogram_samples, resolve_profile,
    write_family_header, write_sample,
};
use core::future::Future;
use core::pin::Pin;
use core::task::{Context, Poll};
use metered::{MetricSchema, MetricValues, SinkError};
use std::fmt::Write;
...
/// Where the renderer is within the grouped document: which family group, and
/// how far into it. Walking group by group keeps each family's lines
/// contiguous (the OpenMetrics grouping rule) exactly like the eager encoder.
#[derive(Clone, Debug)]
enum Cursor {
    Group { group: usize, stage: Stage },
    Eof,
    Done,
}
...
enum Stage {
    ...
    Samples(usize),
    Histograms {
        /// The group's effective histogram render, resolved once per family
        /// group (from the document profile and the group header) when the
        /// stage is entered -- exactly like the eager encoder.
        resolved: HistogramProfile,
        /// The next histogram of this group to expand.
        index: usize,
...
impl<'a> OpenMetricsRender<'a> {
    ...
    pub fn with_profile(
        ...
        OpenMetricsRender {
            groups: group_families(schema, values),
            cursor: Cursor::Group {
                group: 0,
                stage: Stage::Header,
            },
    ...
    pub fn step(
        ...
        out: &mut dyn Write,
        budget: usize,
    ) -> Result<RenderProgress, SinkError> {
        let budget = budget.max(1);
        let mut used = 0;
        while used < budget {
            match &mut self.cursor {
                Cursor::Group { group, stage } => {
                    let Some(current) = self.groups.get(*group) else {
                        self.cursor = Cursor::Eof;
                        continue;
                    };
                    match stage {
                        Stage::Header => {
                            if let Some(family) = current.header {
                                write_family_header(out, family)?;
                                used += 1;
                            }
                            *stage = Stage::Samples(0);
                        }
                        Stage::Samples(index) => match current.samples.get(*index) {
                            Some(sample) => {
                            ...
                            None => {
                                *stage = Stage::Histograms {
                                    resolved: resolve_profile(self.profile, current.header),
                                    index: 0,
                                    pending: Vec::new(),
                                    next: 0,
                                }
                        ...
                        Stage::Histograms {
                            ...
                            if let Some(sample) = pending.get(*next) {
                                ...
                                *next += 1;
                                used += 1;
                            } else if let Some(histogram) = current.histograms.get(*index) {
                                *pending = histogram_samples(histogram, *resolved);
                                *next = 0;
                                *index += 1;
                            } else {
                                let next_group = *group + 1;
                                self.cursor = Cursor::Group {
                                    group: next_group,
                                    stage: Stage::Header,
                                };
                            }
    ...
    pub fn render_to_string(mut self) -> Result<String, SinkError> {
        let mut out = String::new();
...
impl Future for RenderFuture<'_> {
    type Output = Result<String, SinkError>;
...
mod tests {
    ...
    /// Pins the incremental path's per-family-group profile resolution: two
    /// histogram families under one vmrange-capable document, one declaring
    /// `Le` and one defaulting to `Auto`, must each render their own form --
    /// and match the eager encoder byte for byte. A renderer resolving the
    /// profile once per document (or ignoring the group header) fails this.
    #[test]
    fn profile_resolves_per_family_group_like_the_eager_encoder() {
        ...
        let mut schema = MetricSchema::new();
        schema.set_render_for("classic_seconds", HistogramRender::Le);
        schema.add_family("classic_seconds", metered::MetricType::Histogram, &[]);
        schema.add_family("native_seconds", metered::MetricType::Histogram, &[]);
        let mut values = MetricValues::new();
        values.exponential_histogram("classic_seconds", &[], &snapshot);
        values.exponential_histogram("native_seconds", &[], &snapshot);

        let mut expected = String::new();
        {
            let mut encoder = OpenMetricsEncoder::new(&mut expected)
                .histogram_profile(crate::HistogramProfile::VmRange);
            encoder.encode_document(&schema, &values).unwrap();
            encoder.finish().unwrap();
        }

        let mut render =
            OpenMetricsRender::with_profile(&schema, &values, crate::HistogramProfile::VmRange);
        let mut out = String::new();
        while render.step(&mut out, 1).unwrap() == RenderProgress::Pending {}

        assert_eq!(out, expected);
        assert!(
            out.contains("classic_seconds_bucket{le="),
            "the family declaring Le renders le buckets:\n{out}"
        );
        assert!(
            out.contains("native_seconds_bucket{vmrange="),
            "the Auto family renders native vmrange buckets:\n{out}"
        );
    }
