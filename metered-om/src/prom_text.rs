//! Lenient Prometheus-text parsing and the [`TextSourceTree`] passthrough.
//!
...
//! a [`MetricTree`] **without changing a single name,
//! label, or value** -- the foundation for serving metrics from a foreign
//! (older or non-metered) source on a metered scrape endpoint.

use metered::{MetricSampleValue, MetricSchema, MetricTree, MetricValues};
use std::fmt;
...
/// Prometheus-text producer, **without changing names, labels, or values**.
///
...
/// its native trees. Samples are emitted exactly as parsed -- untyped, no
/// `# TYPE` lines, no `_total` normalization -- so existing dashboards keep
/// working unmodified.
///
...
impl TextSourceTree {
    ...
    /// `serde_prometheus` output. Samples pass through byte-for-byte, so e.g. a
    /// metered 0.9 HDR summary (`name{quantile="…"}`, `name_sum`, `name_count`)
    /// re-appears unchanged as a summary-shaped series during migration.
    ///
    /// ```
...
impl MetricTree for TextSourceTree {
    ...
    fn collect(&self, _name: &str, inherited: &[(&str, &str)], values: &mut MetricValues) {
        ...
        for sample in samples {
            ...
            let mut labels: Vec<(&str, &str)> = inherited.to_vec();
            labels.extend(sample.labels.iter().map(|(k, v)| (k.as_str(), v.as_str())));
