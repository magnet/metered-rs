...
//! [`family_by`](metered::MetricTreeView::family_by):
//!
...
//!   `family_by` group advertises its families (with the `rail` label) even before
//!   the first rail is onboarded.
//!
//! [`PaymentRails::metrics_view`] walks the live map at collect time and stamps
//! the rail name on every series via `family_by`, e.g.
...
impl PaymentRails {
    ...
    pub fn register(&self, name: &str) {
        // Fast path: re-registering an existing rail is a no-op, so don't pay
        // for an exclusive lock (or the key allocation) to find that out.
        if self.inner.read().contains_key(name) {
            return;
        }
        // `entry` re-checks under the write lock: a racing caller may have
        // inserted the rail between the read and the write.
        self.inner
            .write()
            .entry(name.to_owned())
...
impl MetricsView for PaymentRails {
    fn metrics_view() -> MetricTreeView<'static, Self> {
        ...
        // `family_by` stamps the `rail` label on each rail's series.
        view.family_by(
            "rail",
...
impl MetricsView for PaymentRail {
    fn metrics_view() -> MetricTreeView<'static, Self> {
        let mut view = MetricTreeView::new();
        // Typed entries declare each family's shape statically, so the `family_by`
