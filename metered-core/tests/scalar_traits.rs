use metered_core::{
    Counter, CounterSource, Gauge, GaugeSource, Info, InfoMetric, Labels, Registry,
};
...
fn registry_registers_external_scalar_trait_values_with_fixed_metric_kinds() {
    ...
    registry.register(
        metered_core::entry::counter("requests")
            .source(&counter)
            .help("Requests"),
    );
    registry.register(
        metered_core::entry::gauge("queue_depth")
            .source(&gauge)
            .help("Queue depth"),
    );
    registry.register(
        metered_core::entry::info("build")
            .source(&info)
            .help("Build info"),
    );
    ...
    assert_eq!(
        ...
        metered_core::MetricType::Counter
    );
    assert_eq!(
        schema.family("queue_depth").unwrap().metric_type,
        metered_core::MetricType::Gauge
    );
    assert_eq!(
        schema.family("build").unwrap().metric_type,
        metered_core::MetricType::Info
    );
