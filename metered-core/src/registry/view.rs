...
pub struct MetricTreeView<'a, C> {
    ...
    /// Registered metric entries. `Send + Sync` so a `'static` view can be
    /// shared across threads or cached in a `static`.
    entries: Vec<Box<dyn entry::MetricEntry<C> + Send + Sync + 'a>>,
...
impl<'a, C: 'a> MetricTreeView<'a, C> {
    ...
    where
        E: entry::MetricEntry<C> + Send + Sync + 'a,
    {
    ...
    where
        M: MetricTree + Sync + 'static,
        F: for<'ctx> Fn(&'ctx C) -> &'ctx M + Send + Sync + 'a,
    {
    ...
    pub fn tree<D>(
        &mut self,
        name: impl Into<Name>,
        select: impl for<'ctx> Fn(&'ctx C) -> &'ctx D + Send + Sync + 'a,
    ...
    where
        D: MetricsView + Sync + 'a,
        F: for<'ctx> Fn(&'ctx C) -> &'ctx D + Send + Sync + 'a,
    {
        self.tree(name, select, D::metrics_view())
    }

    /// Fans out a **dynamic** set of sub-components as one labeled family
    /// group, keyed by a typed [`crate::LabelSet`]: for each live member, the
    /// member's `element_view` metrics are emitted with the key's
    /// `(name, value)` label pairs applied. The dynamic analog of
    /// [`subtree`](Self::subtree), and the tool for service-shaped state that
    /// is itself dynamic (a map of rails, remotes, shards, ...) where the
    /// metrics live *in* the members rather than in a central
    /// [`crate::Family`].
    ///
    /// `iterate` drives the **collect** walk and calls
    /// [`FamilyEmitter::emit`](crate::FamilyEmitter::emit) once per member,
    /// so the caller owns the lock/snapshot scope -- and the emission order:
    /// members render in the order they are emitted (a [`crate::Family`] sorts
    /// its series by label pairs; sort in `iterate` if you want the same).
    /// The **schema** is context-free: it is described from `element_view`
    /// alone, with the key's label **names** declared value-less from
    /// [`crate::LabelSet::label_names`], so the scrape's shape never depends
    /// on which members are live -- an empty group still advertises its
    /// families. That means `L` must declare its label names statically --
    /// `#[derive(LabelSet)]` keys do; fully-dynamic sets such as
    /// `Vec<(String, String)>` do not and are rejected by a `debug_assert!`
    /// at registration. And the element view must declare its metrics through
    /// typed entries (or a directly-held tree): a context-projected sub-tree
    /// inside an element view cannot describe itself and is recorded as a
    /// [`crate::SchemaError::UndescribedEntry`] (surfaced by
    /// [`crate::MetricSchema::validate`]; an element view with *no*
    /// describable entry at all is a `debug_assert!` at registration).
    ///
    /// This is the **borrowed** dual of [`crate::Family`]: your own map is the
    /// storage and the metrics live in the members, where a
    /// `Family<L, M: MetricTree>` owns whole keyed metric structs itself. The
    /// two render identically for the same logical data. When members are
    /// identified by **one string label**, use the
    /// [`family_by`](Self::family_by) sugar instead.
    ///
    /// ```
    ...
    /// #     }
    /// # }
    /// # struct Rails { map: HashMap<RailLabels, Rail> }
    /// let mut view = MetricTreeView::<Rails>::new();
    /// view.family_view(Rail::metrics_view(), |rails: &Rails, out| {
    ///     for (key, rail) in &rails.map {
    ///         out.emit(key, rail);
    ///     }
    /// });
    /// ```
    pub fn family_view<L, D, F>(
        &mut self,
    ...
    where
        L: crate::LabelSet + 'a,
        D: 'a,
        F: Fn(&C, &mut entry::FamilyEmitter<'_, L, D>) + Send + Sync + 'a,
    {
        self.register(entry::family_view(element_view, iterate))
    }

    /// Fans out a **dynamic** set of sub-components as one labeled family
    /// group keyed by **one string label**: for each live member, the member's
    /// `element_view` metrics are emitted with `label` set to its key.
    ///
    /// `family_by` is [`family_view`](Self::family_view) for the common
    /// one-string-key case: both forms feed the same per-member emission
    /// seam, with `[label]` as the group's declared label names. Each
    /// [`Emitter::emit`](crate::Emitter::emit)`(key, element)` stamps its one
    /// `(label, key)` pair **borrowed**, straight off the caller's `&str` --
    /// no key value is copied (a typed [`family_view`](Self::family_view)
    /// key's pairs are materialized instead, because
    /// [`crate::LabelSet::for_each_label`] may lend per-call temporaries).
    /// The schema and element-view rules are exactly
    /// [`family_view`](Self::family_view)'s.
    ///
    /// Note that the first argument is a **label name** (a dimension applied
    /// to every member's series), not a name segment in the metric path like
    ...
    /// let mut view = MetricTreeView::<Rails>::new();
    /// view.family_by("rail", Rail::metrics_view(), |rails: &Rails, out| {
    ///     for (name, rail) in &rails.map {
    ///         out.emit(name, rail);
    ///     }
    /// });
    /// ```
    pub fn family_by<D, F>(
        &mut self,
    ...
    where
        D: 'a,
        F: Fn(&C, &mut entry::Emitter<'_, D>) + Send + Sync + 'a,
    {
        self.register(entry::family_by(label, element_view, iterate))
    }
    ...
    /// Deliberately not gated per entry on
    /// [`needs_housekeep`](entry::MetricEntry::needs_housekeep): for a
    /// tree-shaped entry the needs check walks the same subtree the housekeep
    /// walks, so pre-checking would double the traversal when upkeep is
    /// pending and save nothing when it is not (the walk's leaves gate
    /// themselves -- `LensKind::MAINTAINABLE` plus the metric-level
    /// `needs_housekeep` check -- at the same cost as the needs walk). Whole
    /// clean trees are skipped one level up, where `needs_housekeep_entries`
    /// feeds a tree-level gate (the registry's metric-tree seam,
    /// [`MetricTree::encode`]).
    #[doc(hidden)]
    ...
    pub fn encode(&self, context: &C, sink: &mut dyn MetricSink) -> Result<(), SinkError> {
...
// Hand-written: entries are boxed `dyn MetricEntry` trait objects, so the
// prefix, constant labels, and entry count are the meaningful shape to show.
impl<C> fmt::Debug for MetricTreeView<'_, C> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("MetricTreeView")
