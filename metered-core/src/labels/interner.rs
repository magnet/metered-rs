    ...
    pub fn bound(&self, value: &str) -> Arc<str> {
        {
            ...
            // At capacity, a novel value can only ever resolve to the overflow
            // value, so answer under the read lock: a cardinality flood (every
            // value unseen) must keep recording threads concurrent instead of
            // serializing them on the write lock. The set only grows -- nothing
            // is ever removed -- so a `len >= cap` observation is stable.
        ...
        // Recheck for the race where other writers filled the remaining slots
        // between the read observation above and this write lock.
        if values.len() >= self.cap {
            return self.other.clone();
        }
...
mod tests {
    ...
    #[test]
    fn at_cap_overflow_resolves_without_the_write_lock() {
        // Regression for the cardinality-flood serialization: once at cap, a
        // novel value must resolve to `_OTHER` from the read path alone.
        // Holding a read guard here makes any write-lock attempt in `bound`
        // deadlock, so this test hanging = the fast path regressed.
        let values = BoundedValues::new(2);
        values.bound("a");
        values.bound("b");

        let _read_guard = values.values.read();
        assert_eq!(&*values.bound("novel"), "_OTHER");
        assert_eq!(&*values.bound("a"), "a", "known values still resolve");
    }
