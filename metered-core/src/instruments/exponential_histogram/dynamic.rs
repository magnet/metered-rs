...
impl DynamicExponentialHistogram {
    ...
    fn downscale(&self) {
        ...
        // The sticky `saturated` flag can be a stale signal: a straggling
        // `record` that loaded a table just before its retirement judges
        // density (or fullness) against that retired, dense table and re-sets
        // the flag -- while the *live* table, rebuilt at a coarser schema, may
        // be sparse. Acting on the stale flag would halve the live table's
        // resolution for nothing (and permanently: schema never goes back up).
        // So re-verify density against the live table before rebuilding.
        if old.populated.load(Ordering::Relaxed) < self.downscale_threshold {
            self.saturated.store(false, Ordering::Relaxed);
            return;
        }
        let new_schema = (old.schema - 1).max(MIN_SCHEMA);
        ...
        for slot in old.slots.iter() {
            let index = slot.index.load(Ordering::Acquire);
            if index == EMPTY_SLOT {
                // A straggler publishes a slot in two steps (index CAS, then
                // `count.fetch_add`), and under a weak memory model the count
                // can become visible while the index still reads `EMPTY_SLOT`
                // -- the same reorder `drain_retired` defends against. If we
                // seeded `folded` with the visible count here, that increment
                // would be marked carried without ever being folded anywhere
                // (there is no index to fold it into) and be lost for good. So
                // an index-less slot's `folded` starts at 0: the whole count
                // defers to a later drain, which runs once the index is
                // visible.
                folded.push(AtomicU64::new(0));
                continue;
            }
            let count = slot.count.load(Ordering::Relaxed);
        ...
        // Set the sticky flag BEFORE the retired table is published, so
        // `drain_retired`'s lock-free fast path can never read `false` at a
        // point where a retired table it should have drained was already
        // visible to it (see the ordering argument on `drain_retired`).
        ...
        self.saturated
            .store(still_dense && new_schema > MIN_SCHEMA, Ordering::Relaxed);
    }
    ...
    ///
    /// **Fast path:** most histograms never downscale, so most housekeeps must
    /// not touch the mutex at all. `has_retired` is the lock-free emptiness
    /// mirror: it is stored `true` (Release) in `downscale` *before* the
    /// retired table is pushed (and it is never cleared, matching the vec,
    /// which never shrinks), so in the flag's modification order every retire
    /// is preceded by its `true` store. A drain that loads (Acquire) `false`
    /// therefore skipped nothing it could have been asked to drain: any
    /// concurrent, not-yet-visible retire is drained by the retiring thread
    /// itself later in the same `housekeep` call (its own program order sees
    /// the flag), and by every subsequent scrape -- `needs_housekeep` reads
    /// this same flag, so no residual is ever stranded behind a stale `false`.
...
mod tests {
    ...
    #[test]
    fn downscale_defers_a_slot_whose_index_is_not_yet_visible() {
        // Deterministic regression for the weak-memory lost-update in
        // `downscale`: a straggler's `count.fetch_add` can be visible on a
        // live-table slot while its `index` store is not (still EMPTY_SLOT).
        // The downscale snapshot must NOT seed that slot's `folded` with the
        // visible count -- there is no index to carry it under, so the
        // increment would be marked folded without ever being folded and every
        // later drain would skip it (`count <= folded`). Instead the whole
        // count defers to a drain that runs once the index is visible. We
        // forge that exact interleaving directly instead of racing threads.
        let h = DynamicExponentialHistogram::with_params(6, 16);
        let mut expected = 0u64;
        for k in 0..4000 {
            h.observe(1e-5 * 1.03f64.powi(k % 400));
        ...
        // Forge the reorder on the LIVE table, before the downscale snapshots
        // it: bump an empty slot's count as a straggler's `fetch_add` would,
        // leaving its index unpublished.
        let live = h.table.load_full();
        let slot = live
            .slots
            .iter()
            .find(|slot| slot.index.load(Ordering::Acquire) == EMPTY_SLOT)
            .expect("a 16-wide table below capacity has empty slots");
        slot.count.fetch_add(1, Ordering::Relaxed);
        expected += 1;

        // The downscale retires the table; the buggy snapshot would mark the
        // forged increment as already folded here, losing it permanently.
        // Settle the schema so later housekeeps do not swap again mid-test.
        while h.needs_rescale() {
            h.rescale_if_needed();
        }

        // Index still not visible -> the residual stays deferred, not lost.
        h.housekeep();
        assert_eq!(
            h.count(),
            expected - 1,
            "an index-less residual must stay deferred, unclaimed"
        );

        // Publish the index (completing the straggler); the next drain folds
        // the deferred increment exactly once.
        publish_forged_index(&h);
        h.housekeep();
        assert_eq!(
            h.count(),
            expected,
            "the straggler's increment must be folded once its index appears"
        );
        h.housekeep();
        assert_eq!(h.count(), expected, "a later drain must not double-count");
    }

    #[test]
    fn stale_saturated_flag_does_not_downscale_a_sparse_live_table() {
        // Deterministic regression for the stale-flag spurious downscale: a
        // straggling `record` that loaded a table just before its retirement
        // can judge density against that (dense) retired table and re-set the
        // sticky `saturated` flag, even though the live table -- rebuilt at a
        // coarser schema -- is sparse. Acting on that stale flag would halve
        // the live table's resolution permanently. `downscale` must re-verify
        // density against the live table and clear the flag instead.
        let h = DynamicExponentialHistogram::with_params(6, 16);
        for k in 0..4000 {
            h.observe(1e-5 * 1.03f64.powi(k % 400));
            if h.needs_rescale() {
        ...
        let live = h.table.load_full();
        assert!(
            live.populated.load(Ordering::Relaxed) < h.downscale_threshold,
            "precondition: the settled live table is sparse"
        );
        let settled_schema = h.schema();

        // The straggler's spurious re-set: it saw the retired table as
        // dense/full and stored `true`, exactly as `record`'s slow path does.
        h.saturated.store(true, Ordering::Relaxed);
        ...
        assert_eq!(
            h.schema(),
            settled_schema,
            "a sparse live table must not be downscaled on a stale flag"
        );
        assert!(
            !h.needs_rescale(),
            "the stale flag is cleared once density is re-verified"
        );
    }
    ...
    #[test]
    fn housekeep_skips_the_retired_mutex_when_nothing_was_ever_retired() {
        // Regression for the ungated drain: a histogram that never downscaled
        // must not touch the `retired` mutex on housekeep. Holding the mutex
        // here makes any lock attempt in `drain_retired` deadlock, so this
        // test hanging = the lock-free fast path regressed.
        let h = DynamicExponentialHistogram::with_params(6, 16);
        h.observe(1.0);
        let _guard = h.retired.lock();
        h.housekeep();
        assert_eq!(h.count(), 1);
    }
