...
/// without a breaking change. Third-party
/// [`ExponentialHistogram`](crate::ExponentialHistogram) backends construct
/// one through [`ExponentialBucket::new`].
...
impl ExponentialBucket {
    /// Assembles a bucket from its parts; [`lower_bound`] / [`upper_bound`]
    /// compute the `lower` / `upper` boundaries for an `index` at a `schema`.
    pub fn new(index: i32, lower: f64, upper: f64, count: u64, exemplar: Option<Exemplar>) -> Self {
        ExponentialBucket {
            index,
            lower,
            upper,
...
/// without a breaking change. Third-party
/// [`ExponentialHistogram`](crate::ExponentialHistogram) backends construct
/// one through [`ExponentialSnapshot::new`].
#[derive(Clone, Debug, PartialEq)]
#[non_exhaustive]
...
impl ExponentialSnapshot {
    ...
    /// [`ExponentialHistogram`](crate::ExponentialHistogram) backends.
    /// `buckets` must hold only populated buckets, ascending by index, and
    /// `count` must equal `zero_count + overflow_count` plus every bucket's
    /// count.
    pub fn new(
        schema: i32,
        zero_count: u64,
        buckets: Vec<ExponentialBucket>,
        overflow_count: u64,
        sum: f64,
        count: u64,
    ) -> Self {
...
mod tests {
    ...
    /// The public constructors (the third-party `ExponentialHistogram`
    /// backend path) build exactly the snapshot the built-in assembly
    /// produces.
    #[test]
    fn snapshot_constructors_round_trip_with_the_internal_assembly() {
        let assembled = ExponentialSnapshot::from_buckets(3, 1, 2, 4.5, [(0, 3, None)]);
        let constructed = ExponentialSnapshot::new(
            3,
            1,
            vec![ExponentialBucket::new(
                0,
                lower_bound(0, 3),
                upper_bound(0, 3),
                3,
        ...
        assert_eq!(assembled, constructed);
    }
