...
//!
//! Unlike metered 0.9's HDR histogram, this histogram does **not** compute
...
#[must_use = "a `Buckets` boundary set does nothing until given to a histogram"]
pub struct Buckets {
...
impl Buckets {
    ...
    /// `+Inf` bucket -- always [`Buckets::bounds`]`.len() + 1`, so a `Buckets`
    /// is never empty. (Deliberately not named `len`: a `len`/`is_empty` pair
    /// would violate their contract, since this count can never be zero.)
    #[must_use]
...
///
/// Deliberately exhaustive: an exemplar is a user-constructed *input* --
/// every [`ExemplarSource`] impl builds one with a struct literal -- so
/// construction ergonomics win over field evolvability. These fields are the
/// contract; adding one would be a breaking change and will not happen in
/// 0.10.x.
#[derive(Clone, Debug, PartialEq)]
pub struct Exemplar {
...
/// span, and any recorder reading [`ThreadLocalExemplars`] as its
/// [`ExemplarSource`] picks it up automatically. It is opt-in because, unlike
/// the rest of the crate, it relies on ambient state.
#[cfg(feature = "exemplar-context")]
pub use self::context::{ThreadLocalExemplars, set_current_exemplar, with_exemplar};
...
mod context {
    ...
    /// value, so a recorder using it as its source attaches the active trace's
...
///
/// `#[non_exhaustive]`: a future OpenMetrics bucket attribute can become a new
/// field without a breaking change (the exemplar field itself arrived that
/// way). Third-party [`Histogram`](crate::Histogram) backends construct one
/// through [`Bucket::new`].
#[derive(Clone, Debug, PartialEq)]
#[non_exhaustive]
pub struct Bucket {
...
impl Bucket {
    /// Assembles a bucket from its parts -- the constructor for third-party
    /// [`Histogram`](crate::Histogram) backends building their snapshot.
    pub fn new(le: f64, cumulative_count: u64, exemplar: Option<Exemplar>) -> Self {
        Bucket {
            le,
            cumulative_count,
            exemplar,
...
/// format.
///
/// `#[non_exhaustive]`: a future snapshot attribute (a `created` timestamp,
/// min/max) can become a new field without a breaking change. Third-party
/// [`Histogram`](crate::Histogram) backends construct one through
...
impl HistogramSnapshot {
    /// Assembles a snapshot from its parts -- the constructor for third-party
    /// [`Histogram`](crate::Histogram) backends. `buckets` must be cumulative,
    /// ascending by `le`, and end with the `+Inf` bucket whose count equals
    /// `count`.
...
mod tests {
    ...
    /// The public constructors (the third-party `Histogram` backend path)
    /// build exactly the snapshot the built-in backend produces.
    #[test]
    fn snapshot_constructors_round_trip_with_a_recorded_backend() {
        let recorded = BucketHistogram::new(Buckets::custom([1.0]));
        recorded.observe(0.5);
        recorded.observe(2.0);

        let constructed = HistogramSnapshot::new(
            vec![
                Bucket::new(1.0, 1, None),
                Bucket::new(f64::INFINITY, 2, None),
            ],
            2.5,
            2,
        );
        assert_eq!(recorded.snapshot(), constructed);
    }
    ...
    fn custom_sorts_dedups_and_drops_invalid() {
        ...
        assert_eq!(b.bounds(), &[0.005, 0.01, 0.025]);
        assert_eq!(b.bucket_count(), 4); // 3 finite + Inf
    }
    ...
    fn one_millisecond_lands_in_fast_one_milli_bucket() {
        // A known 1ms op must land in the right sub-5ms bucket when using the
        // fast preset (the OTel default would dump it into le=0.005).
    ...
    fn thread_local_exemplar_source_reads_ambient_context() {
        use super::{ExemplarSource, ThreadLocalExemplars, with_exemplar};
