...
///
/// # `incr` ambiguity with [`Counter`](crate::Counter)
///
/// `AtomicU64` implements both [`Counter`](crate::Counter) and `Gauge`, and
/// both traits have an `incr` method. With **both traits in scope**, a plain
...
/// Gauge::incr(&depth);   // the gauge reading
/// Counter::incr(&depth); // the counter reading
/// ```
///
/// With only one of the traits in scope, `depth.incr()` resolves normally.
...
/// `AtomicU64` as a gauge.
///
/// `AtomicU64` also implements [`Counter`](crate::Counter), so when both
/// traits are in scope call `Gauge::incr(&value)` (see the [`Gauge`] trait
/// docs on the `incr` ambiguity).
