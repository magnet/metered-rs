...
///
/// `#[non_exhaustive]`: a future bucket attribute can become a new field
/// without a breaking change. Third-party [`GaugeHistogramSource`] impls
/// construct one through [`GaugeBucket::new`].
...
impl GaugeBucket {
    ...
    /// [`GaugeHistogramSource`] impls building their snapshot.
    pub fn new(le: f64, cumulative_count: i64) -> Self {
        GaugeBucket {
            le,
            cumulative_count,
        }
...
///
/// `#[non_exhaustive]`: a future snapshot attribute can become a new field
/// without a breaking change. Third-party [`GaugeHistogramSource`] impls
/// construct one through [`GaugeHistogramSnapshot::new`].
...
impl GaugeHistogramSnapshot {
    ...
    /// [`GaugeHistogramSource`] impls. `buckets` must be cumulative, ascending
    /// by `le`, and end with the `+Inf` bucket whose count equals `gcount`.
    pub fn new(buckets: Vec<GaugeBucket>, gsum: f64, gcount: i64) -> Self {
...
impl<S: GaugeHistogramSource> Metric for GaugeHistogram<S> {
    ...
    fn describe_metric(&self, name: &str, labels: &[(&str, &str)], schema: &mut MetricSchema) {
        // Structural path: check incoming labels for a user `le` before
        // compose would shadow it under the type exemption.
        schema.add_family_structural(name, MetricType::GaugeHistogram, labels, &[("le", "")]);
    }
}
...
impl GaugeBuckets {
    ...
    ///
    /// A `NaN` value is dropped entirely (no bucket, no `gsum`), matching the
    /// crate-wide NaN discipline: one NaN would otherwise poison `gsum`
    /// forever.
    ...
    /// ≥ net leaves or the counts go negative. A `NaN` value is dropped, the
    /// mirror of [`enter`](GaugeBuckets::enter)'s NaN handling.
...
mod tests {
    ...
    /// The public constructors (the third-party `GaugeHistogramSource` path)
    /// build exactly the snapshot the built-in instrument produces.
    #[test]
    fn snapshot_constructors_round_trip_with_a_recorded_instrument() {
        let hist = GaugeBuckets::new([1.0]);
        hist.enter(0.5);
        hist.enter(2.0);

        let constructed = GaugeHistogramSnapshot::new(
            vec![GaugeBucket::new(1.0, 1), GaugeBucket::new(f64::INFINITY, 2)],
            2.5,
            2,
        );
        assert_eq!(hist.snapshot(), constructed);
    }
    ...
    #[test]
    fn nan_enter_and_leave_are_dropped_and_do_not_poison_gsum() {
        let hist = GaugeBuckets::new([1.0, 2.0]);
        hist.enter(1.5);
        hist.enter(f64::NAN);
        hist.leave(f64::NAN);
        ...
        let snap = hist.snapshot();
        // The NaN never reaches a bucket (it would land in bucket 0) or gsum
        // (which one NaN would poison forever).
        assert_eq!(snap.gcount, 1);
        assert!(snap.gsum.is_finite());
        assert_eq!(snap.gsum, 1.5);
        assert_eq!(snap.buckets[0].cumulative_count, 0);
    }
    ...
    #[test]
    fn user_le_label_on_a_gauge_histogram_is_reserved() {
        // A user/inherited `le` must not be erased by the structural compose
        // + type exemption: validate must see ReservedLabel.
        let gh = GaugeHistogram::new(GaugeBuckets::new([1.0]));
        let mut schema = MetricSchema::new();
        gh.describe_metric("queue_size", &[("le", "1"), ("shard", "a")], &mut schema);
        assert_eq!(
            schema.validate().unwrap_err(),
            vec![crate::schema::SchemaError::ReservedLabel {
                name: "queue_size".to_owned(),
                label: "le".to_owned(),
            }]
        );
        assert!(
