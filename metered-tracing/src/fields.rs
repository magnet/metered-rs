...
/// The union of span-field names the registered consumers of a span (its
/// recorders and the exemplar provider) actually read at close.
///
/// Built once at layer construction and consulted by the capture visitor
/// *before* any allocation or `Debug` formatting: a field nobody reads is
/// dropped by name, so a span carrying a large debug payload pays nothing for
/// it. A consumer whose field set is open-ended (a hand-written
/// [`SpanRecorder`](crate::SpanRecorder), [`FromSpanFields`](crate::FromSpanFields),
/// or [`ExemplarProvider`](crate::ExemplarProvider) that does not declare its
/// names) widens the demand to [`All`](FieldDemand::All), keeping full capture
/// for that configuration only.
#[derive(Clone, Debug)]
pub(crate) enum FieldDemand {
    /// Some consumer is open-ended over field names: capture every field.
    All,
    /// Only these field names are read; every other field is rejected before
    /// capture.
...
impl FieldDemand {
    fn wants(&self, name: &str) -> bool {
        match self {
            FieldDemand::All => true,
            FieldDemand::Named(names) => names.contains(name),
        }
    }

    /// Extends this demand with one consumer's declared field names, or
    /// widens it to [`All`](FieldDemand::All) when the consumer is
    /// open-ended (`None`).
    pub(crate) fn extend(&mut self, fields_read: Option<Vec<String>>) {
        match (self, fields_read) {
            (FieldDemand::All, _) => {}
            (demand, None) => *demand = FieldDemand::All,
            (FieldDemand::Named(names), Some(read)) => names.extend(read),
        }
    }
}

impl Default for FieldDemand {
    /// Nothing demanded: every field is rejected.
    fn default() -> Self {
        FieldDemand::Named(HashSet::new())
    }
}
...
///
/// A field outside `demand` is rejected by name before any capture work: no
/// key/value allocation, and -- decisive for `record_debug` -- no `Debug`
/// formatting of a value nobody reads.
pub(crate) struct SpanFieldVisitor<'a> {
    fields: &'a mut SpanFields,
    demand: &'a FieldDemand,
}

impl<'a> SpanFieldVisitor<'a> {
    pub(crate) fn new(fields: &'a mut SpanFields, demand: &'a FieldDemand) -> Self {
        SpanFieldVisitor { fields, demand }
    }

    fn wants(&self, field: &Field) -> bool {
        self.demand.wants(field.name())
    }
}
...
mod tests {
    ...
    #[test]
    fn field_demand_unions_names_and_widens_for_open_ended_consumers() {
        let mut demand = FieldDemand::default();
        assert!(!demand.wants("rpc.method"), "nothing demanded by default");

        demand.extend(Some(vec!["rpc.method".to_owned()]));
        assert!(demand.wants("rpc.method"));
        assert!(!demand.wants("payload"), "undeclared fields stay rejected");

        // An open-ended consumer (no declared names) widens to capture-all.
        demand.extend(None);
        assert!(demand.wants("payload"));

        // Once widened, further named consumers cannot narrow it back.
        demand.extend(Some(vec!["rpc.method".to_owned()]));
        assert!(demand.wants("anything.else"));
    }
