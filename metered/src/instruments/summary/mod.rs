...
// Exposed (doc-hidden) so the `metered-semantic` migration module can derive a
// legacy summary from a histogram's buckets without duplicating the math.
#[doc(hidden)]
...
// Exposed (doc-hidden) for the `metered-semantic` migration module.
#[doc(hidden)]
...
impl<S: QuantileSource> Metric for Summary<S> {
    ...
    fn describe_metric(&self, name: &str, labels: &[(&str, &str)], schema: &mut MetricSchema) {
        let all = with_labels(labels, [("quantile", "")]);
        schema.add_family(name, MetricType::Summary, &all);
    }
}
