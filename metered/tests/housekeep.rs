...
#[test]
fn family_view_forwards_housekeep_to_members() {
    ...
    // The maintainable metric lives *in* the member, reachable only through
    // the group's element view: upkeep must flow through `iterate`, or a
    // saturated dynamic histogram inside a member would never rescale.
    struct Shard {
        requests: AtomicU64,
    ...
    element.register(metered::entry::counter("requests").select(|s: &Shard| &s.requests));
    element.register(metered::entry::metric("latency").select(|s: &Shard| &s.latency));

    let mut view = MetricTreeView::with_prefix("shards");
    view.family_view(element, |shards: &HashMap<ShardLabels, Shard>, out| {
    ...
    assert!(
        !view.needs_housekeep_entries(&shards),
        "a clean group reports no upkeep"
    );

    // Saturate the member's histogram so a downscale is pending.
    let shard = &shards[&key];
    let start_schema = shard.latency.schema();
    for k in 0..4000 {
        shard.latency.observe(1e-5 * 1.03f64.powi(k % 400));
        if shard.latency.needs_rescale() {
            break;
        }
    }
    assert!(
        shard.latency.needs_rescale(),
        "precondition: the member's histogram needs a rescale"
    );

    assert!(
        view.needs_housekeep_entries(&shards),
        "a member's pending upkeep must propagate through family_view"
    );
    view.housekeep(&shards);
    assert!(
        shards[&key].latency.schema() < start_schema,
        "housekeep must rescale the histogram inside the emitted member"
    );
}
