...
use metered::{MetricSchema, MetricTree, MetricType, MetricValues, join_name};
...
/// Saturating microseconds conversion: a `Duration` past `u64::MAX` micros
/// (585k+ years) clamps instead of silently truncating.
...
const TASK_COUNTERS: &[TaskCounter] = &[
    ...
    // The exported names deliberately drop tokio_metrics' `total_` field
    // prefix: these are OpenMetrics counters, so the encoder already renders
    // the `_total` suffix (`..._poll_total`, not `..._total_poll_total`).
    TaskCounter {
        name: "first_poll_delay_microseconds",
    ...
    TaskCounter {
        name: "idle_duration_microseconds",
    ...
    TaskCounter {
        name: "scheduled_duration_microseconds",
    ...
    TaskCounter {
        name: "poll_duration_microseconds",
    ...
    TaskCounter {
        name: "fast_poll_duration_microseconds",
    ...
    TaskCounter {
        name: "slow_poll_duration_microseconds",
...
mod tests {
    ...
    async fn task_metrics_emit_instrumented_future_counts() {
        ...
        assert!(text.contains("# UNIT tokio_task_poll_duration_microseconds microseconds"));
        assert!(text.contains("tokio_task_instrumented_total 1"));
        assert!(text.contains("tokio_task_first_poll_total 1"));
        assert!(text.contains("tokio_task_poll_total 2"));
    }
    ...
    fn schema_declares_all_task_metric_families() {
        ...
        assert!(
            schema
                .family("tokio_task_scheduled_duration_microseconds")
                .is_some()
        );
