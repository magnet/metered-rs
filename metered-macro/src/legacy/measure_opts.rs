...
pub struct MeasureRequest<'a> {
    ...
    // Parsed from the `debug = ...` measure option but not yet consumed by
    // codegen. Retained so the option keeps parsing; see issue #52.
    #[allow(dead_code)]
...
impl Parse for NonEmptyMeasureRequestAttribute {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        ...
        let this = NonEmptyMeasureRequestAttribute { inner };
...
impl MeasureRequestKeyValAttribute {
    fn validate(&self, input: ParseStream<'_>) -> Result<()> {
        self.values
            ...
            .ok_or_else(|| {
                input.error(
                    "missing `type` attribute with a path to a valid metered::Measure struct.",
                )
            })?;
