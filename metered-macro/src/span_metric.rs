...
//! Each field type must be `FromFieldValue + Default` (and `Display` for eager
//! fields).
//!
...
fn parse_decl(input: &DeriveInput) -> syn::Result<Decl> {
    ...
    let (span_name, help_attr) = parse_container(input)?;
    let help = help_attr.or_else(|| doc_string(&input.attrs));
...
/// Parses the container `#[span(name = "...", help = "...")]` attribute.
fn parse_container(input: &DeriveInput) -> syn::Result<(LitStr, Option<String>)> {
    let attr = input
        .attrs
        .iter()
...
fn codegen(decl: &Decl) -> TokenStream2 {
    ...
    let from_fields = fields.iter().map(|f| {
        ...
        let fallback = match &f.default {
            Some(default) => quote! {
                <#ty as ::metered_tracing::FromFieldValue>::from_text(#default)
                    .map_err(|error| ::metered_tracing::SpanFieldsError {
                        field: #otel,
                        error,
                    })?
            },
        ...
        quote! {
            #ident: match fields.value(#otel) {
                ::core::option::Option::Some(__value) => {
                    <#ty as ::metered_tracing::FromFieldValue>::from_field_value(__value)
                        .map_err(|error| ::metered_tracing::SpanFieldsError {
                            field: #otel,
                            error,
                        })?
    ...
    let record_fns = fields.iter().filter(|f| f.on_close).map(|f| {
        ...
        quote! {
            #[doc = #doc]
            pub fn #fn_name(span: &::metered_tracing::__rt::tracing::Span, value: #ty) {
                ::metered_tracing::__rt::tracing::Span::record(
                    span,
                    #otel,
                    ::metered_tracing::__rt::tracing::field::display(&value),
                );
            }
        }
    });
    ...
    let on_close_fields: Vec<TokenStream2> = fields
        ...
        .map(|f| {
            let otel = &f.otel;
            quote! { #otel = ::metered_tracing::__rt::tracing::field::Empty, }
        })
        .collect();
    ...
    // struct" footgun -- and usable by dependent crates. It deliberately shares
    // the labels type's name (the type and the macro are in different
    // namespaces); `metered_info_span!(Type; ...)` dispatches to it.
    let emitter = quote! {
        #[macro_export]
        macro_rules! #name {
            (@open #matcher) => {{
                #(#eager_lets)*
                ::metered_tracing::__rt::tracing::info_span!(
                    #span_name,
                    #(#eager_fields)*
                    #(#on_close_fields)*
                    #extra_expand
    ...
    quote! {
        ...
        impl ::metered_tracing::FromSpanFields for #name {
            fn try_from_span_fields(
                fields: &::metered_tracing::SpanFields,
            ) -> ::core::result::Result<Self, ::metered_tracing::SpanFieldsError> {
