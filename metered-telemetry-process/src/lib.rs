...
//! bounded and non-allocating, so [`collect`](MetricTree::collect) is safe to
//! call from an async scrape handler. There is no background task and nothing to
//! keep warm.
//!
//! [`metrics-process`]: https://crates.io/crates/metrics-process

use metered::{
    join_name, Help, MetricSampleValue, MetricSchema, MetricTree, MetricType, MetricValues,
};
